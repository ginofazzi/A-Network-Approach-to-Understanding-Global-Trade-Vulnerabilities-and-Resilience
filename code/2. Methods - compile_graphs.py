...
labels = pd.read_csv(data_paths["labels"], dtype={"product_code": str})
...
if multi_graph:
  ...
  for year in range(2012, 2021):
    ...
    g = load_graph(graph_name=f"{year}-{graphs_type}", labels=labels.loc[(labels.year==year), ["country_id", "label"]], label_map=label_map, \
          root_path=f"{data_paths['graphs_data']}{'/multi-graph/' if multi_graph else '/'}{graphs_type}")
  ...
  for i, prod_code in enumerate([f"{x:02d}" for x in range(1, 100)]):
      ...
      train_graphs += load_data(years=list(range(2012,2021)), code=prod_code, labels=labels, label_map=label_map, suffix=graphs_type, \
                                  root_path=f"{data_paths['graphs_data']}/{graphs_type}")
...
#save_object(train_graphs, f"../data/graphs_data{'/multi-graph/' if multi_graph else '/'}{graphs_type}/train-graphs.pkl")
#save_object(test_graphs, f"../data/graphs_data{'/multi-graph/' if multi_graph else '/'}{graphs_type}/test-graphs.pkl")