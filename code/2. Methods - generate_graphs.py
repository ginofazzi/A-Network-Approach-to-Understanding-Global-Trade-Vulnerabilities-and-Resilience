...
trustworhiness = pd.read_csv(f"{data_paths['graphs_data']}/trustworthiness_scores.csv", dtype={"cmd": str})
geo_embeddings = pd.read_csv("geo-embeddings.vec", sep=" ", skiprows=1, header=None)
...
for product_code in [f"{x:02d}" for x in range(1, 100)]:
    ...
    for year in range(2012,2023):
        ...
        node_features.to_csv(f"../data/graphs_data/{transaction}/node_features-{year}-{product_code}-{transaction}.csv", index=False)
        edges.to_csv(f"../data/graphs_data/{transaction}/edge_features-{year}-{product_code}-{transaction}.csv", index=False)
