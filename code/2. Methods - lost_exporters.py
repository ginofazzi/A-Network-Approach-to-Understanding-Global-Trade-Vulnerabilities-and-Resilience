...
products = pd.read_csv("../data/atlas/product_hs12.csv")
...
for year in years[:-1]:
    ...
    for ix, row in rca_matrix.iterrows():
        ...
        if len(importers) == 0:
            continue
        #subset = df[(df.country_id.isin(importers)) & (df.product_code==pid) & (df.year.isin([year, year+1])) & (df.import_value > 0)]\
        ...
        #subset = pd.DataFrame({"year": [year, year+1]}).merge(subset, on="year", how="left").fillna(0)
        ...
        #subset = pd.pivot_table(data=subset[["country_id", "year", "export_value"]], values="export_value", columns="year", index="country_id")
        #display(subset)
        #break
        i += 1
...
df_impact_drop_exporters.to_csv("df_impact_drop_exporters.csv", index=False)